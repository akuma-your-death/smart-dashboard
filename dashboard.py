...
while routers_website != "ip" or routers_website[:12] != ip_for_proof:
    ...
    if routers_website == "ip":
        print(ip)
    if routers_website != "ip" and routers_website[:12] != ip_for_proof:
        print("thats not a correct mode write (ip or your standard gateway)")
    routers_website = input("whats your standard gateway? ")
    if routers_website[:12] == ip_for_proof:
        with open("routers_ip.txt", "w") as file:
